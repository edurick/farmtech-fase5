"""Gera tabelas/gráfico de custos e documentos com os resultados verificados."""
...
def moeda(x): return Decimal(str(x)).quantize(Decimal('.01'), rounding=ROUND_HALF_UP)
regioes = {'Virgínia do Norte': {'codigo':'us-east-1','ec2':{'t4g.micro':'.0084','t3a.micro':'.0094','t3.micro':'.0104'}},
           'São Paulo': {'codigo':'sa-east-1','ec2':{'t4g.micro':'.0134','t3a.micro':'.0151','t3.micro':'.0168'}}}
linhas=[]
for reg, val in regioes.items():
    catalogo=json.loads((ROOT/f"docs/aws/precos-oficiais-{val['codigo']}.json").read_text())
    for disco,sufixo in [('Magnetic','EBS:VolumeUsage'),('gp3 SSD','EBS:VolumeUsage.gp3')]:
        tarifa=next(Decimal(r['PricePerUnit']) for r in catalogo['linhas'] if r['usageType'].endswith(sufixo))
        for instancia,hora in val['ec2'].items():
            ec2=Decimal(hora)*730; ebs=tarifa*50; total=ec2+ebs
            linhas.append({'regiao':reg,'codigo':val['codigo'],'instancia':instancia,'disco':disco,
                           'usd_hora':float(hora),'ec2_mensal_exato':float(ec2),'ebs_mensal':float(ebs),
                           'total_mensal_exato':float(total),'total_mensal_calculadora':float(moeda(total)),
                           'total_anual_calculadora':float(moeda(total)*12),
                           'total_anual_sem_arredondamento_mensal':float(moeda(total*12))})
custos=pd.DataFrame(linhas)
custos.to_csv(ROOT/'docs/aws/comparacao_custos.csv',index=False)
# Tabela e gráfico derivam dos mesmos números, sem transcrição manual divergente.
sns.set_theme(style='whitegrid',font_scale=1.1)
fig,axes=plt.subplots(1,2,figsize=(11,4.8),sharey=True)
for ax,disco in zip(axes,['Magnetic','gp3 SSD']):
    sub=custos[(custos.instancia=='t4g.micro') & (custos.disco==disco)]
    x=range(len(sub)); ax.bar(x,sub.ec2_mensal_exato,color='#23435a',label='EC2 Linux · 730 h')
    ax.bar(x,sub.ebs_mensal,bottom=sub.ec2_mensal_exato,color='#248775',label='EBS · 50 GB')
    for i,(_,r) in enumerate(sub.iterrows()):
        ax.text(i,r.total_mensal_exato+.35,f'US$ {r.total_mensal_calculadora:.2f}',ha='center',weight='bold')
    ax.set_xticks(list(x),['Virgínia do Norte','São Paulo']); ax.set_title(disco,weight='bold')
    ax.set_ylim(0,21); ax.set_ylabel('Custo básico por mês (USD)'); ax.legend(loc='upper left',fontsize=9)
fig.suptitle('Mesma máquina, duas regiões · t4g.micro · 100% On-Demand',weight='bold')
fig.text(.5,.01,'Cotação: 08/09/2026 · sem impostos, IPv4 público, tráfego ou créditos excedentes · Magnetic: I/O cobrado à parte',ha='center',fontsize=9)
plt.tight_layout(rect=[0,.05,1,.94]); fig.savefig(ROOT/'docs/aws/comparacao-custos.png',dpi=180,bbox_inches='tight'); plt.close(fig)
link=(ROOT/'docs/aws/link-comparacao.txt').read_text().strip()
link_hd=(ROOT/'docs/aws/link-magnetic.txt').read_text().strip()
r=json.loads((ROOT/'resultados/resumo.json').read_text())
readme=f'''# FarmTech Solutions — Fase 5
...
[AWS Pricing Calculator]({link}). Nenhum recurso foi contratado ou implantado.
...
[Reabrir a estimativa somente com HD nas duas regiões]({link_hd}).
...
- [Comparação principal com Magnetic, duas regiões]({link_hd}).
- [Comparação completa com HD e SSD]({link}). **Cada linha é uma alternativa. O total geral soma cenários e não representa a solução escolhida.**
...
print('README, CSV e gráfico de custos gerados.')
